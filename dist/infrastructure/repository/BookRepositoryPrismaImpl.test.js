...
import { BookRepositoryPrismaImpl } from './BookRepositoryPrismaImpl.js';
...
describe('BookRepositoryPrismaImpl', () => {
    ...
    let prismaMock;
    beforeEach(() => {
        // Clear mock instances and create a fresh repository which will instantiate the mocked PrismaClient
        PrismaClient.mockClear();
        repo = new BookRepositoryPrismaImpl();
        // Use the shared mock client exposed by the mock
    ...
    test('createLoanForBook runs transaction and creates borrower when absent', async () => {
        ...
        // borrower not found -> will be created
        ...
        await repo.createLoanForBook({ bookId: '3', borrowerName: '新規利用者', borrowerEmail: 'a@example.com' });
        expect(txMock.book.findUnique).toHaveBeenCalledWith({ where: { id: 3 } });
        expect(txMock.borrower.findFirst).toHaveBeenCalledWith({ where: { name: '新規利用者' } });
        expect(txMock.borrower.create).toHaveBeenCalledWith({ data: { name: '新規利用者', email: 'a@example.com' } });
