...
describe('BookRepositoryPrismaImpl', () => {
  ...
  beforeEach(() => {
    ...
    // construct issuer with mocked clients and pass issuer to repo
    issuer = new ClientIssuer(__mockClient, __mockClient);
    repo = new BookRepositoryPrismaImpl(issuer);
  ...
  test('createLoanForBook runs transaction and creates borrower when absent', async () => {
    ...
    const createdRow = {
      id: 3,
      title: 'Created Book',
      bookAuthors: [{ author: { name: '作成者' } }],
      isbn: null,
      published: new Date(),
      description: null,
      loans: [],
    };
    prismaMock.book.findUnique.mockResolvedValue(createdRow);
